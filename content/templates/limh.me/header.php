...
<header id="header">
    <div class="box">
        <div class="logo"><a rel="index" title="<?php echo $blogname; ?>" href="<?php echo BLOG_URL; ?>"><img
                        src="<?php if (_g('logoqq') == 1) {
                            echo myhk_qq(_g('qqhao'));
                        } else {
                            echo _g('logo');
                        } ?>" alt="<?php echo $blogname; ?>"/></a></div>
        <h1><a title="网站首页"
               href="<?php echo BLOG_URL; ?>"><?php echo $site_title; ?></a><?php if (_g('zbzy') == 1): ?><?php if (str_replace("?_pjax=%23lmhblog", "", BLOG_URL . trim(Dispatcher::setPath(), '/')) == BLOG_URL): ?>
                <div class="header-v" title="<?php echo _g('zbjs'); ?>"></div><?php endif; ?><?php endif; ?></h1>
        <div class="text">
            <ul>
                <?php global $CACHE;
                $newtws_cache = $CACHE->readCache('newtw'); ?><?php foreach ($newtws_cache as $value): ?>
                    <li><a title="查看更多微言碎语" href="<?php echo BLOG_URL . 't/'; ?>"><i
                                    class="fa fa-twitter"></i> <?php echo date('Y年n月j日 - ', $value['date']);
                        echo preg_replace('/\[F(([1-4]?[0-9])|50)\]/', '', $value['t']); ?></li></a>
                <?php endforeach; ?>
            </ul>
        </div>
    </div>
